...
for idx_h = 1:n_intervals
    ...
    for idx_tau = 1:(length(delays)-1)
        tau = delays(idx_tau);
        stab = test_stability(SYS,h,tau,{K1_static});
...
[stab_K1_static, unstab_K1_static] = stable_unstable_pairs(data_K1_static);

%%
figure;
scatter(stab_K1_static(:,1),stab_K1_static(:,2),[],"black","o","filled"); hold on;
scatter(unstab_K1_static(:,1),unstab_K1_static(:,2),1,[0.8,0.8,0.8],"."); hold on;
